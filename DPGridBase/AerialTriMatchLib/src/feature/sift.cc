...
void
Sift::set_image (dpgrid::ByteImage::ConstPtr img)
{
    if (img->channels() != 1 && img->channels() != 3)
        throw std::invalid_argument("Gray or color image expected");

    this->orig = dpgrid::image::byte_to_float_image(img);
    if (img->channels() == 3)
    {
        this->orig = dpgrid::image::desaturate<float>
            (this->orig, dpgrid::image::DESATURATE_AVERAGE);
    }
}
...
void
Sift::set_float_image (dpgrid::FloatImage::ConstPtr img)
{
    if (img->channels() != 1 && img->channels() != 3)
        throw std::invalid_argument("Gray or color image expected");
    ...
    {
        this->orig = dpgrid::image::desaturate<float>
            (img, dpgrid::image::DESATURATE_AVERAGE);
    }
    else
    {
...
Sift::create_octaves (void)
{
    ...
    if (this->options.min_octave < 0)
    {
        ...
        dpgrid::FloatImage::Ptr img
            = dpgrid::image::rescale_double_size_supersample<float>(this->orig);
     ...
     */
    dpgrid::FloatImage::ConstPtr img = this->orig;
    for (int i = 0; i < this->options.min_octave; ++i)
        img = dpgrid::image::rescale_half_size_gaussian<float>(img);
...
void
Sift::add_octave (dpgrid::FloatImage::ConstPtr image,
        float has_sigma, float target_sigma)
{
    ...
    dpgrid::FloatImage::Ptr base = (target_sigma > has_sigma
        ? dpgrid::image::blur_gaussian<float>(image, sigma)
        : image->duplicate());
    ...
    for (int i = 1; i < this->options.num_samples_per_octave + 3; ++i)
    {
        ...
        dpgrid::FloatImage::Ptr img = dpgrid::image::blur_gaussian<float>
            (base, blur_sigma);
        oct.img.push_back(img);

        /* Create the Difference of Gaussian image (DoG). */
        dpgrid::FloatImage::Ptr dog = dpgrid::image::subtract<float>(img, base);
...
Sift::extrema_detection (void)
{
    ...
    for (std::size_t i = 0; i < this->octaves.size(); ++i)
    {
        ...
        {
            dpgrid::FloatImage::ConstPtr samples[3] =
...
std::size_t
Sift::extrema_detection (dpgrid::FloatImage::ConstPtr s[3], int oi, int si)
{
...
Sift::keypoint_localization (void)
{
    ...
    for (std::size_t i = 0; i < this->keypoints.size(); ++i)
    {
        ...
        dpgrid::FloatImage::ConstPtr dogs[3] =
...
Sift::generate_grad_ori_images (Octave* octave)
{
    ...
    {
        dpgrid::FloatImage::ConstPtr img = octave->img[i];
        dpgrid::FloatImage::Ptr grad = dpgrid::FloatImage::create(width, height, 1);
        dpgrid::FloatImage::Ptr ori = dpgrid::FloatImage::create(width, height, 1);
    ...
    Octave const* octave, std::vector<float>& orientations)
{
    ...
    dpgrid::FloatImage::ConstPtr grad(octave->grad[is + 1]);
    dpgrid::FloatImage::ConstPtr ori(octave->ori[is + 1]);
