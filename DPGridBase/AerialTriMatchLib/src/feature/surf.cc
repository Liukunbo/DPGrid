...
void
Surf::set_image (dpgrid::ByteImage::ConstPtr image)
{
    /* Desaturate input image. */
    if (image->channels() != 1 && image->channels() != 3)
        throw std::invalid_argument("Gray or color image expected");
    if (image->channels() == 3)
        image = dpgrid::image::desaturate<uint8_t>(image,
            dpgrid::image::DESATURATE_LIGHTNESS);

    /* Build summed area table (SAT). */
    //util::WallTimer timer;
    this->sat = dpgrid::image::integral_image<uint8_t,SatType>(image);
