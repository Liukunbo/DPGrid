 ...
 */
#ifndef FEA_SIFT_HEADER
#define FEA_SIFT_HEADER
...
public:
    ...
    /** Sets the input image. */
    void set_float_image (dpgrid::FloatImage::ConstPtr img);
...
protected:
    ...
    struct Octave
    {
        typedef std::vector<dpgrid::FloatImage::Ptr> ImageVector;
...
protected:
    ...
    void add_octave (dpgrid::FloatImage::ConstPtr image,
        float has_sigma, float target_sigma);
    void extrema_detection (void);
    std::size_t extrema_detection (dpgrid::FloatImage::ConstPtr s[3],
        int oi, int si);
...
private:
    Options options;
    dpgrid::FloatImage::ConstPtr orig; // Original input image
