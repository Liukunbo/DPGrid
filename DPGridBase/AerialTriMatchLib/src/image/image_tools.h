#ifndef DPG_IMAGE_TOOLS_HEADER
#define DPG_IMAGE_TOOLS_HEADER
...
template <typename T>
void
find_min_max_value (typename dpgrid::Image<T>::ConstPtr image, T* vmin, T* vmax);
...
template <typename T>
void
reduce_alpha (typename dpgrid::Image<T>::Ptr img); // TODO Blend color?
 ...
 */
template <typename T>
typename dpgrid::Image<T>::Ptr
sobel_edge (typename dpgrid::Image<T>::ConstPtr img);
...
template <typename T>
void
reduce_alpha (typename dpgrid::Image<T>::Ptr img)
{
...
sobel_edge (typename dpgrid::Image<T>::ConstPtr img)
{
    ...
    typename dpgrid::Image<T>::Ptr out = dpgrid::Image<T>::create
        (width, height, chans);
    T* out_ptr = out->get_data_pointer();
    ...
    int thumb_width, int thumb_height)
{
    ...
    typename dpgrid::Image<T>::Ptr thumb = dpgrid::image::rescale<T>(image,
        dpgrid::image::RESCALE_LINEAR, rescale_width, rescale_height);
    thumb = dpgrid::image::crop<T>(thumb, thumb_width, thumb_height,
